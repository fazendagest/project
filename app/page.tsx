...
export default async function MarketplacePage({
  ...
  return (
    <div className="min-h-screen" style={{ background: '#FAF8F3' }}>
      ...
      {/* ── Header ── */}
      <header className="bg-[#166534] text-white sticky top-0 z-40 border-b border-green-800">
        <div className="max-w-7xl mx-auto px-4 py-3 flex items-center gap-3">
          ...
          <form method="GET" action="/" className="flex-1 max-w-2xl">
            <div className="flex items-center bg-white rounded-xl px-3 gap-2 shadow-sm">
              ...
              <button
                type="submit"
                className="bg-[#166534] text-white text-sm font-semibold px-4 py-1.5 rounded-lg hover:bg-green-800 transition-colors shrink-0"
              >
                Buscar
              </button>
            </div>
          ...
          <div className="flex items-center gap-2 shrink-0">
            ...
            <Link href="/anunciar" className="text-sm font-semibold bg-white text-[#166534] px-3 py-1.5 rounded-full hover:bg-green-50 transition-colors flex items-center gap-1">
      ...
      {/* ── Hero strip ── */}
      <div className="bg-[#166534] text-white border-b border-green-800">
      ...
      {/* ── Category bar ── */}
      <div className="bg-white border-b border-[#E8E3D6] sticky top-[60px] z-30">
                ...
                <Link
                  ...
                  className={`shrink-0 flex items-center gap-1.5 px-3.5 py-1.5 rounded-full text-sm font-medium border transition-colors ${
                    active
                      ...
                      : 'text-gray-700 border-[#E8E3D6] hover:border-[#166534] hover:text-[#166534]'
                  }`}
                >
      ...
      <div className="max-w-7xl mx-auto px-4 py-8 space-y-10">
        ...
        {showEvents && (
          <section>
            <div className="flex items-baseline justify-between mb-4">
              ...
              <Link href="/eventos/novo" className="text-sm text-[#166534] font-semibold flex items-center gap-1">
              ...
              {events!.map(event => {
                ...
                return (
                  <Link
                    ...
                    className="shrink-0 w-56 bg-white rounded-xl border border-[#E8DFC9] overflow-hidden hover:shadow-md transition-shadow flex flex-col"
                    style={{ height: 196 }}
                  >
                    {/* Cover */}
                    <div className="relative h-[88px] flex items-center justify-center overflow-hidden" style={{ background: '#E6EFD9' }}>
                      <div className="absolute top-2 left-2 bg-white rounded-lg overflow-hidden border border-green-200 shadow-sm" style={{ width: 44, height: 48 }}>
                        <div className="bg-[#166534] text-white text-center text-[8.5px] font-bold uppercase tracking-widest py-0.5">{month}</div>
                        <div className="flex items-center justify-center flex-1 h-[30px] text-[#166534] font-bold text-lg font-serif">{day}</div>
                      </div>
                      <span className="absolute top-2 right-2 bg-white text-[#166534] text-[10px] font-semibold px-2 py-0.5 rounded-full border border-green-200 shadow-sm">
                        {event.type}
                      </span>
                    </div>
                    {/* Body */}
            ...
            <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 gap-4">
              {listings.map(listing => (
                <Link key={listing.id} href={`/anuncio/${listing.id}`} className="group">
                  <div className="bg-white rounded-xl border border-[#E8E3D6] overflow-hidden hover:shadow-lg hover:-translate-y-0.5 transition-all duration-150 h-full flex flex-col">
                    {/* Photo */}
                    ...
                    <div className="p-3 flex-1 flex flex-col">
                      ...
                      <div className="flex items-baseline justify-between mt-2">
                        <p className="font-semibold text-[#166534] text-base font-serif">
                          {formatPrice(listing.price, listing.price_type)}
                        </p>
                        <span className="text-[10.5px] text-gray-400">{timeAgo(listing.created_at)}</span>
                      </div>
                      {listing.seller_name && (
                        <p className="text-[11px] text-gray-500 mt-1.5 pt-1.5 border-t border-[#E8E3D6] flex items-center gap-1 truncate">
                          Por{' '}
                          <span className="font-medium text-[#1F1A12] truncate">{listing.seller_name}</span>
                          <Shield className="h-3 w-3 text-[#166534] shrink-0" />
                        </p>
                      )}
                    </div>
                  </div>
        ...
        <section
          ...
          style={{ background: '#F5EFE2', border: '1px solid #E8DFC9' }}
        >
          <div className="absolute right-0 bottom-0 opacity-[0.06] pointer-events-none">
            <svg viewBox="0 0 280 280" width="280" height="280">
              <path d="M280 0 C280 154 154 280 0 280 L0 0 Z" fill="#166534" />
            </svg>
          </div>
          <div className="p-8 lg:p-10 relative">
            ...
            <p className="text-sm text-gray-700 mt-3 mb-6 max-w-lg leading-relaxed">
              ...
              <strong className="text-[#166534]">90 dias grátis</strong>, sem cadastrar cartão.
              Funciona até com sinal fraco no celular.
            </p>
            <div className="flex items-center gap-4 flex-wrap">
              <Link
                href="/register"
                className="bg-[#166534] text-white font-semibold px-5 py-2.5 rounded-xl hover:bg-green-800 transition-colors flex items-center gap-2 text-sm"
              >
                Experimentar grátis <ArrowRight className="h-4 w-4" />
              </Link>
            </div>
            <p className="text-xs text-gray-500 mt-3 flex items-center gap-1.5">
              <svg className="h-3.5 w-3.5 shrink-0" style={{ color: '#166534' }} viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5" strokeLinecap="round" strokeLinejoin="round">
      ...
      {/* ── Footer ── */}
      <footer className="bg-[#166534] text-white mt-4">
